...
feedback_gen = 1000			# number of maxGen between feedback
feedback_time = 0.0			# pause in sec every feedback_gen epochs
...
maxGen=1;				# max number of epochs (generations!) to run the GA
popSize=4;					# size of the population
crossProp=0.00;				# crossover propability
mutProp=0.00;				# mutation propability
elitePport=0.10;			# proportion of best from both parents and children kept
...
for generation in range(maxGen):
    ...
    for j in range(int(popSize/2)):
        ...
        #np.concatenate((offspring,child1),axis=1)
        offspring = np.concatenate((offspring,child1))
        offspring = np.concatenate((offspring,child2),axis=1)
        #offspring[j*2] = child1
        #offspring[j*2+1] = child2
        print "child1",child1
        print "child2",child2
